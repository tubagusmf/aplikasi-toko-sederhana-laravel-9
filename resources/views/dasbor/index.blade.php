        ...
        <tr>
            ...
            <td>
                <?php if($header_transaksi->status_bayar == 'Sudah') { ?>
                    <span class="btn btn-success"><i class="fa fa-check-circle"></i> <?php echo $header_transaksi->status_bayar ?></span>
                <?php } elseif($header_transaksi->status_bayar == 'Menunggu') { ?>
                    <span class="btn btn-info"><i class="fa fa-clock"></i> <?php echo $header_transaksi->status_bayar ?></span>
                <?php } elseif($header_transaksi->status_bayar == 'Konfirmasi') { ?>
                    <span class="btn btn-info"><i class="fa fa-clock"></i> <?php echo $header_transaksi->status_bayar ?></span>
                <?php } elseif($header_transaksi->status_bayar == 'Dibatalkan') { ?>
                    <span class="btn btn-secondary"><i class="fa fa-times-circle"></i> <?php echo $header_transaksi->status_bayar ?></span>
                <?php } ?>
            </td>
            <td>
                ...
                <a href="{{ asset('admin/transaksi/delete/'.$header_transaksi->id_header_transaksi) }}" class="btn btn-dark btn-sm mb-1 delete-link"><i class="fa fa-trash"></i></a>
            </td>
        </tr>
        <?php $i++; } ?>
    </tbody>
